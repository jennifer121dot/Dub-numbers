...
async function getServicesWithCache(country) {
const key = `services_${country}`;
const cached = serviceCache.get(key);

if (cached && (Date.now() - cached.timestamp) < SERVICE_CACHE_TTL) {
console.log(`📦 Using cached services for ${country} (${cached.data.length} services)`);
return cached.data;
}

console.log(`🔄 Fetching fresh services for ${country} from 5SIM...`);
const data = await fivesimGetServices(country);
serviceCache.set(key, { data, timestamp: Date.now() });
console.log(`✅ Cached ${data.length} services for ${country}`);
return data;
}
...
return {
...svc,
price: parseFloat(priceInNgn.toFixed(2)),
currency: 'NGN',
...
clearServiceCache();
res.json({ success: true, message: 'Service cache cleared' });
});
...
logger.info(`💾 Service cache: enabled (5 minute TTL)`);
logger.info(`🔗 Database pool: 100 connections`);
});
